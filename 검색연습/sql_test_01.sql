...
-- 문제2. 전체직원의 다음 정보를 조회하세요. 가장 선임부터 출력이 되도록 하세요. 출력은 이름, 성별, 입사일 순서이고
--       "이름", "성별", "입사일"로 컬럼 이름을 대체 해보세요
...
select count(distinct emp_no)  as 직원 
from salaries;
...
select count(distinct emp_no) as 매니저수
from dept_manager;


-- 문제7 전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보세요

select distinct dept_name as 부서이름
from departments
order by length(dept_name) desc;

-- 문제8 현재 급여가 120000이상 받는 사원은 몇 명이나 있습니까?

select count(distinct emp_no) as 사원수
from salaries
where to_date like '9999-%' and salary >=120000;

-- 문제9 어떤 직책들이 있나요? 중복 없이 이름이 긴 순서대로 출력해 보세요.
select distinct(title) as 직책이름
from titles
